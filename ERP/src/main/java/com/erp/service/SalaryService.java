...
//    }
    ...
    public void markAsPaid(Long id) {
        ...
        if (salary.getEmployee() != null) {
            ...
            payslip.setDateIssued(salary.getMonth() != null ? salary.getMonth() : LocalDate.now());
    ...
    public SalaryDashboardSummaryDto getSalaryDashboardSummary() {
        ...
        Long pendingPayslipsCount = salaryRepository.countPendingHrApproval();
        Long forwardedPayslipsCount = salaryRepository.countForwardedPending();

        return new SalaryDashboardSummaryDto(totalPayout, pendingPayslipsCount, forwardedPayslipsCount);
    }
    ...
    public List<MyPayslipDto> getMyPayslips(String email) {
        ...
        return salaryRepository.findByEmployeeId(employee.getId()).stream()
                ...
                .sorted(Comparator.comparing(Salary::getMonth).reversed())
                .map(s -> new MyPayslipDto(
                        s.getId(),
                        s.getMonth().getMonth().getDisplayName(TextStyle.FULL, Locale.ENGLISH),
                        s.getMonth().getYear(),
                        s.getTotalPayable(),
                        s.getStatus(),
                        "/api/payslip/payslip/" + s.getId() + "/download/self"))
                .collect(Collectors.toList());
    }
    ...
    public String getEmployeeEmailBySalaryId(Long id) {
        ...
        return employeeRepository.findAll().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new RuntimeException("Employee email not found for salary"));
    }

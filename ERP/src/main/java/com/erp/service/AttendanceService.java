...
public class AttendanceService {
    ...
    public Attendance markAttendance(AttendanceDto attendanceDto) {
        ...
        if (attendanceDto.getStatus() == AttendanceStatus.LEAVE) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "LEAVE status cannot be set manually. Approve a leave request instead.");
        }
    ...
    private boolean isOnApprovedLeave(Long employeeId, LocalDate date) {
        return employeeLeaveRepository.existsByEmployeeIdAndStartDateLessThanEqualAndEndDateGreaterThanEqualAndStatus(
                employeeId, date, date, EmployeeLeave.LeaveStatus.APPROVED);
    }
 ...
 // Method to convert Attendance entity to AttendanceResponseDto
    ...
    public Attendance saveAttendance(AttendanceDto dto) {
        ...
        boolean isOnLeave = employeeLeaveRepository.existsByEmployeeIdAndStartDateLessThanEqualAndEndDateGreaterThanEqualAndStatus(
            empId, date, date, EmployeeLeave.LeaveStatus.APPROVED);

        if (isOnLeave) {
            throw new InvalidStatusException("Attendance cannot be marked. Employee is on approved leave.");
        }

        if (dto.getStatus() == AttendanceStatus.LEAVE) {
            throw new InvalidStatusException("LEAVE status cannot be set manually. Approve a leave request instead.");
        }
        ...
        Attendance attendance = Attendance.builder()
                ...
                .present(dto.getStatus().isCountedAsPresent())
...
//    }
    ...
    private YearMonth parseYearMonth(String monthStr) {
        ...
        String normalized = monthStr.length() >= 10 ? monthStr.substring(0, 10) : monthStr;

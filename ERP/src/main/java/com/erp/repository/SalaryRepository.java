...
//	List<Salary> findByMonthAndYear(@Param("month") int month, @Param("year") int year);
    ...
    List<Salary> findByMonth(LocalDate month);

    @Query("SELECT COUNT(s) FROM Salary s WHERE s.approvedByHR = false")
    Long countPendingHrApproval();

    @Query("SELECT COUNT(s) FROM Salary s WHERE s.forwardedToFinance = true AND s.paid = false")
    Long countForwardedPending();
